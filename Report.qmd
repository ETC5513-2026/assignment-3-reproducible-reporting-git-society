...
We also ran into a quirk with the director data. Some films on IMDB have more than one director listed, but since the dataset wasn't structured consistently to handle that, only the first director mentioned for each film was scraped into the director variable. The tricky part is that the extra directors didn't just disappear - they ended up being misclassified as actors in the dataset instead. This is worth keeping in mind when looking at the director and actor results, as it may have affected how accurate those findings are.
...
From @tbl-success-stats, it appears that gross box office revenue is highly skewed, where the average gross revenue is much greater than the median value, which indicates that the measure is dominated by a few blockbuster films. In contrast, the IMDb ratings have low variability with close mean and median values, reflective of the dataset as it only contains the top 1000 films on IMDb.
...
-   `Genre` - Average gross revenue compared across genres and average IMDB rating compared across genres

-   `Runtime` - Average IMDB rating and average gross revenue compared across runtime groups

-   `Metascore` - Relationship between critic score and IMDB rating examined using a scatter plot

-   `Number of votes` - Relationship between vote count and IMDB rating examined using a scatter plot

-   `Director` - Top directors ranked by average IMDB rating and average gross revenue

-   `Actor` - Top actors ranked by average IMDB rating and average gross revenue

-   `Release year` - Distribution of films across release years examined to identify temporal patterns in the dataset
...
Studios aiming to achieve stronger critical and audience ratings may benefit from focusing on genres that are more strongly associated with narrative depth and character-driven storytelling. While the results show that high-revenue genres such as Adventure and Sci-Fi do not necessarily correspond to higher IMDb ratings, more traditional or narrative-focused genres such as Western, War, and Film-Noir are often associated with stronger emphasis on storytelling, emotional complexity, and character development, which may contribute to higher audience appreciation. Additionally, collaborating with experienced directors in these genres may also help improve the overall quality and reception of the film.

**For Studios Prioritising Box Office Revenue**\

Studios focused on commercial success should continue to invest in Adventure, Science Fiction, and Action films. These genres often attract wider audiences and are well suited to large-scale production, franchise development, sequels, and strong marketing campaigns.
